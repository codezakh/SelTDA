    ...
    def forward(
        ...
        if train:
            ...
            if inference == "generate":
                ...
                max_ids, max_scores = self.rank_answer(
                    question_output.last_hidden_state,
                    question.attention_mask,
                    answer.input_ids,
                    answer.attention_mask,
                    k_test,
                    return_top_probability=True,
                )
                if return_scores:
                    return max_ids, max_scores
                return max_ids

    def rank_answer(self, question_states, question_atts, answer_ids, answer_atts, k, return_top_probability=False):
        ...
        if return_top_probability:
            return max_ids, log_probs_sum.max(dim=1).values
        return max_ids
