...
class LowResConfig:
    ...
    low_res_images_dir: Path = Path(
        "/net/acadia4a/data/zkhan/rsvqa/low_resolution/Images_LR"
    )
...
def convert_rsvqa_lr_record_tuple_to_vqa_format(
    question: LRQuestionRecord, answer: LRAnswerRecord
) -> Tuple[schemas.TrainingRecord, schemas.MinimalEvaluationRecord]:
...
def transform_split_into_std_format(split: RsVqaLrSplit) -> StandardFormatSplit:
    ...
    for q, a in zip(split.questions, split.answers):
        (
            training_record,
            evaluation_record,
        ) = convert_rsvqa_lr_record_tuple_to_vqa_format(q, a)
        training_records.append(training_record)
        evaluation_records.append(evaluation_record)

    return StandardFormatSplit(
        training_records=training_records, testing_records=evaluation_records
    )
...
if __name__ == "__main__":
    ...
    logger.info(
        "Loaded %d answers and %d questions",
        len(lr_dump.all_answers),
        len(lr_dump.all_questions),
    )
    ...
    write_json(
        lr_config.output_annotations.train,
        [_.dict() for _ in lr_train.training_records],
    )
    write_json(
        lr_config.output_annotations.test, [_.dict() for _ in lr_test.training_records]
    )
    write_json(
        lr_config.output_annotations.test_annotations,
        [_.dict() for _ in lr_test.testing_records],
    )
