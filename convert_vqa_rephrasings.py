...
def point_record_to_coco_image_file(
    record: schemas.TrainingRecord, split: str
...
if __name__ == "__main__":
    val_questions_raw = load_json(
        VQA_REPHRASINGS_ROOT / "v2_OpenEnded_mscoco_valrep2014_humans_og_questions.json"
    )

    logger.info("Converting %d records", len(val_questions_raw["questions"]))

    val_questions = [
        make_val_record(schemas.QuestionRecord.parse_obj(_))
        for _ in val_questions_raw["questions"]
    ]

    logger.info("Verifying all image paths are correct.")
    val_records = [
        point_record_to_coco_image_file(r, "val") for r in tqdm(val_questions)
    ]
    ...
    logger.info(
        "Wrote %d validation records to %s",
        len(val_records),
        VQA_REPHRASINGS_ROOT / "val.json",
    )
