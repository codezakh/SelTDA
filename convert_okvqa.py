...
class OkvqaSplit(BaseModel):
    ...
    @classmethod
    def build_from_path(cls, split_name: str, annotation_path: Path, question_path: Path):
        logger.info(f'Loading {split_name} split from {annotation_path} and {question_path}')
        with open(annotation_path, 'r') as f:
            annotations = json.load(f)['annotations']
        with open(question_path, 'r') as f:
            questions = json.load(f)['questions']
        logger.info('Structuring %d annotations and %d questions', len(annotations), len(questions))
        annotations = [schemas.VQAAnnotationRecord.parse_obj(a) for a in tqdm(annotations)]
        questions = [schemas.QuestionRecord.parse_obj(q) for q in tqdm(questions)]
        assert len(annotations) == len(questions)
        return cls(split=SplitType(split_name), annotations=annotations, questions=questions)
...
def make_train_record(annotation: schemas.VQAAnnotationRecord, question: schemas.QuestionRecord) -> schemas.TrainingRecord:
...
def point_record_to_coco_image_file(record: schemas.TrainingRecord, split: SplitType) -> schemas.TrainingRecord:
    coco_image_id = pad_coco_id(record.image)
    relative_path_to_image = f"{split.value}2014/COCO_{split.value}2014_{coco_image_id}.jpg"
...
if __name__ == '__main__':
    ...
    logger.info('Converting %d annotations and questions into training records', len(train.annotations))
    train_records = [make_train_record(a, q) for a, q in tqdm(zip(train.annotations, train.questions))]

    logger.info('Verifying all %d images are present', len(train_records))
    train_records = [point_record_to_coco_image_file(r, train.split) for r in tqdm(train_records)]
    ...
    val_records = [point_record_to_coco_image_file(r, val.split) for r in tqdm(val_records)]
    ...
    logger.info('Wrote %d training records to %s', len(train_records), OKVQA_ROOT / 'train.json')

    with open(OKVQA_ROOT / 'val.json', 'w') as f:
        json.dump([r.dict() for r in val_records], f)
    logger.info('Wrote %d validation records to %s', len(val_records), OKVQA_ROOT / 'val.json')