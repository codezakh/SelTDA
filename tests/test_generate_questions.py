...
def test_parsing_rationale():
    ...
    record = VQARecord.build_from_raw_model_output(raw_model_output, '/fake/path/to/image.jpg', parse_rationale=True)
    assert record.answer ==  [_.strip() for _ in
        "trees, forest, savanna, park, forest, woods, grassland, zoo, zoo, zoo".split(',')
    ]
    assert record.question == "what does this animal live in?"
    assert record.rationale == "the animals in this photo are found in the savannah. there is a small group of animals that appear to have some unique features. the animal on the left seems to be an elephant or tiger."
...
def test_parsing_output_with_no_rationale():
    ...
    record = VQARecord.build_from_raw_model_output(raw_model_output, '/fake/path/to/image.jpg', parse_rationale=False)
...
def test_parsing_question_when_no_colon_first():
    ...
    record = VQARecord.build_from_raw_model_output(raw_model_output, '/fake/path/to/image.jpg', parse_rationale=False)
    assert record.answer ==  [_.strip() for _ in
        "waiting, waiting, waiting, waiting, waiting, waiting, waiting, waiting, waiting, waiting".split(',')
    ]
